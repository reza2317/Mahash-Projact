...
export const InteractiveCalendar: React.FC<InteractiveCalendarProps> = ({
  ...
  return (
    <div className={`space-y-6 ${className}`} id="interactive-calendar-section">
      ...
      {/* 1. Header & Controls */}
      <div className="bg-white dark:bg-slate-850 rounded-3xl p-4 sm:p-6 border border-slate-200 dark:border-slate-800 shadow-sm space-y-5 transition-colors">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="px-3.5 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-bold transition flex items-center gap-1.5 cursor-pointer"
            >
            ...
            <div className="bg-slate-100 dark:bg-slate-800 p-1 rounded-xl flex items-center gap-1 border border-slate-200 dark:border-slate-700">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition flex items-center gap-1.5 cursor-pointer ${
                  viewMode === 'calendar'
                    ? 'bg-white dark:bg-slate-700 text-[#173b82] dark:text-blue-300 shadow-xs'
      ...
      {viewMode === 'calendar' ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-850 rounded-3xl p-5 sm:p-6 border border-slate-200 dark:border-slate-800 shadow-sm space-y-5 transition-colors">
            ...
            {/* Weekday Labels */}
            <div className="grid grid-cols-7 gap-1 text-center font-black text-xs text-slate-400 dark:text-slate-500 pb-2 border-b border-slate-100 dark:border-slate-800">
                ...
                return (
                  ...
                  >
                    ...
                    {hasEvents ? (
                      ...
                      <span className="text-[9px] font-bold text-amber-500">امروز</span>
                    ) : null}
                  </button>
                );
              })}
          ...
          <div className="lg:col-span-5 space-y-4">
            ...
            {displayedEvents.length === 0 ? (
              <div className="bg-white dark:bg-slate-850 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 text-center space-y-3">
                <div className="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 mx-auto flex items-center justify-center text-xl">
                  📅
                </div>
              ...
              <div className="space-y-3 max-h-[620px] overflow-y-auto pr-1">
                {displayedEvents.map((event) => (
                  <div
                    key={event.id}
                    className="bg-white dark:bg-slate-850 rounded-3xl p-5 border border-slate-200 dark:border-slate-800 shadow-xs hover:shadow-md transition-all space-y-3"
                  >
                    {/* Badge & Date */}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {filteredEvents.map((event) => (
              <div
                key={event.id}
                className="bg-white dark:bg-slate-850 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-all flex flex-col justify-between space-y-4"
              >
            ...
            <div className={`p-6 bg-gradient-to-r ${activeEventModal.coverGradient || 'from-blue-600 to-indigo-800'} text-white relative`}>
              ...
              <div className="space-y-2">
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/20 text-white text-xs font-bold backdrop-blur-xs">
                  <span>{activeEventModal.icon}</span>
                ...
                <>
                  ...
                  <form onSubmit={handleRegisterSubmit} className="space-y-4 pt-3 border-t border-slate-100 dark:border-slate-800">
                    <h5 className="text-sm font-black text-slate-900 dark:text-slate-100 flex items-center gap-2">
                      <Ticket className="w-4 h-4 text-[#0f766e] dark:text-teal-400" />
